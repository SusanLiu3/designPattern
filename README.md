...
#### 装饰者模式
- 概述：不改变对象自身情况下，动态给某个对象添加额外的职责
#### 状态模式
- 概述：将状态封装成单独的类，并将请求委托给当前的状态对象；我们使用的对象，在不同的状态下具有截然不同的行为，这个对象是从不同的类中创建的，实际使用了委托。
- 优缺点：
    - 优点：
        - 状态模式定义了状态和行为的关系，并封装在一个类里；新增和切换新的状态很容易
        - 状态切换的逻辑封装在状态类里，省去了ifelse分支
        - 使用对象代替字符串来区分不同状态，使状态切换更加一目了然
        - 对象的请求和状态的封装互相独立互不影响
    - 缺点：会在程序中定义很多状态类，逻辑分散，无法看出整个状态机的逻辑
#### 适配器模式
概述：解决两个软件实体的接口不兼容问题，通过转换使他们可以协同工作
### 设计模式原则
#### 单一职责原则（SRP）
- 被定义为“引起变化的原因” （如 单例模式、装饰者模式）
- 优点：降低了单个类或对象的复杂度，按照职责将对象进行更小粒度的细分；有利于代码复用
- 缺点：增加了代码编写的复杂度，加大了对象之间的联系
#### 最少知识原则（迪米特法则）
- 尽可能减少对象之间的联系（如中介者模式）
#### 开放-封闭原则（OCP）
- 当需要修改程序的功能或者新增一个新功能时，采用新增代码的方式实现（装饰者模式）
### 代码重构
#### 提炼代码
- 避免函数过大
- 独立出来的函数有助于代码复用
